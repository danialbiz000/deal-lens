...
The MVP's single blended tranche (§4) is still the default. `LboInputs.debt_tranches` is an optional tuple of `DebtTranche(name, leverage_multiple, interest_rate, mandatory_amort_pct, priority)`; leaving it unset reconstructs exactly one implicit tranche from the existing scalar `lbo_leverage_multiple`/`lbo_interest_rate`/`lbo_mandatory_amort_pct` fields, so every pre-existing caller and stored `LBOCase` is byte-identical to before this feature existed (enforced by `test_no_tranches_given_is_byte_identical_to_the_old_single_blended_behavior`).

**Per-tranche mechanics, every year:**
- **Interest**: each tranche accrues on its own beginning-of-year balance at its own `interest_rate`, independent of every other tranche. Total interest (used in the tax-shield calc, unchanged from §4) is the sum across tranches.
- **Mandatory amortization**: each tranche pays down `mandatory_amort_pct` of its *own original principal* (not the deal's total), capped at its own current balance. A tranche with `mandatory_amort_pct=0` (typical for subordinated/mezzanine debt) never amortizes on its own.
- **Cash sweep**: the total sweep pool is computed exactly as in §4 (unchanged), then allocated across tranches strictly in `priority` order (lower first) — a tranche receives zero sweep cash until every higher-priority tranche's balance (net of that year's own mandatory amort) is fully repaid. This mirrors a real credit agreement's mandatory-prepayment waterfall: sweep priority is a contractual term, not something inferred from interest rate.

Aggregate scalars (`ScheduleYear.interest/mandatory_amort/sweep/ending_debt`) are always the sum across tranches, so every existing consumer (API responses already in production, the value creation bridge, entry/exit leverage) is unaffected by whether tranches are used. `ScheduleYear.tranches: Optional[List[TrancheYear]]` additionally exposes the full per-tranche breakdown when more than one tranche is in play.
